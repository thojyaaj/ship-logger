...
import { getProblemShipments, type ProblemScan, type ShippingLossScan } from "@/lib/shipment-alerts";
        ...
        <>
          {losses.length > 0 && (
            <section className="flex flex-col gap-2">
              <h2 className="tag-label !text-base !text-red-ink">Shipping losses — paid more than charged</h2>
              <LossTable items={losses} />
            </section>
          )}
...
function LossTable({ items }: { items: ShippingLossScan[] }) {
  return (
        ...
        <tbody>
          {items.map((i) => (
            <tr key={i.id} className="border-t border-line bg-paper-panel">
              ...
              <td className="px-3 py-2 data font-semibold !text-red-ink">
                -{formatMoney(i.lossAmount, i.costCurrency)}
              </td>
              <td className="px-3 py-2 data truncate">
